...
template<class T, int SZ> struct LazySeg { 
	...
	void upd3(int v, int pos, int val, int L=0, int R=SZ-1){//WRONG HERE
		if(L==R){
			seg[pos]=val;
		} else{
			int M=(L+R)/2;
			if(pos<=M)
				upd3(v*2, pos, val, L, M);
			else
				upd3(v*2+1, pos, val, M+1, R);
