...
void solve(){
    ...
    For(i,n)FOR(j,i+1,n)FOR(k,j+1,n){
        vpi v;
        v.pb(p[i]);v.pb(p[j]);v.pb(p[k]);
        sort(all(v));
        ll tmp=cross(v[0],v[2],v[1]);
        assert(tmp!=0);
        if(tmp>0){
            int tcnt=m[mp(v[0],v[1])]+m[mp(v[1],v[2])]-m[mp(v[0],v[2])];
            // assert(tcnt>=0);
            cnt[tcnt]++;
        }
        else{
            int tcnt=m[mp(v[0],v[2])]-m[mp(v[1],v[2])]-m[mp(v[0],v[1])];
            // assert(tcnt>=0);
            // dbg(i,j,k,tcnt,v,m[mp(v[0],v[2])],m[mp(v[1],v[2])],m[mp(v[0],v[1]));
            cnt[tcnt-1]++;
        }
    }
    For(i,n-2)ps(cnt[i]);
