...
ll comb(int a, int b) {
    if (a < b || b < 0) return 0;
    return (ll)fac[a]*modPow(fac[b],MOD-2)%MOD*modPow(fac[a-b],MOD-2)%MOD;
}


ll n, M;
ll cnt[MX], dp[MX][MX][2], c[MX][MX];
void go(int x, int prim){
    ...
    FOR(i,1,n+1)FOR(j,1,n+1){
        ...
        For(gr,i/j+1){
            ...
            For(k,2){
                ...
                tmp=mul(tmp,c[i][space]);//choose from
                tmp=mul(tmp,modPow(modPow(j,gr),MOD-2));//rotations
                tmp=mul(tmp,modPow(fac[gr],MOD-2),MOD);
                tmp=mul(tmp,fac[space]);
                if(x==4)dbg(fac[space],i,j,gr,space,comb(i,space));
                dp[i][j][w|k]+=tmp;
                dp[i][j][w|k]%=MOD;   
            }
        }
    ...
    if(x==4){
        For(i,n+1)For(j,n+1)For(k,2)dbg(i,j,k,dp[i][j][k]);
    }
    cnt[x]=dp[n][n][1];
}
...
void solve(){
    ...
    M=MOD;
    MOD--;
    c[1][0] = c[1][1] = 1;//choose function since MOD-1 isnt prime
    for (int i = 2; i < MX; i++) {
        c[i][0] = c[i][i] = 1;
    ...
    FOR(i,2,n+1)if(cnt[i]){
        ret=mul(ret,modPow(i,cnt[i],M),M);
        dbg(i,cnt[i]);
    }
    dbg(c[3][1]);
    ps(ret);
