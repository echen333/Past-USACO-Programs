...
ll findAns(ll val){
    ...
    while(!pq.empty() && cnt<=k && room>=0){
        ...
        if(p[i]<sz(b[i])-1)
            pq.push(mp(b[i][p[i]+1],i));//-b[i][p[i]]
        room-=b[i][p[i]];if(room<0)break;
        dbg(i,p[i],b[i]);
        dbg(x,i,p[i],room,(--m.ub(room))->s,bit.sum((--m.ub(room))->s));
        cnt+=bit.sum((--m.ub(room))->s);
        cval+=bit2.sum((--m.ub(room))->s);
        dbg("HERE",bit2.sum((--m.ub(room))->s));
        FOR(j,p[i]+1,sz(b[i])){
            bit.upd(m[b[i][j]],1);
            bit2.upd(m[b[i][j]],b[i][j]);
        }
