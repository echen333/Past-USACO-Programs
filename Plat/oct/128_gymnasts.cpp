//
// Created by Edward on 10/28/2020.
//
 ...
 * for PIE, start from larger and see if smaller included
 * can also use totient function to find number of relatively prime
 * try impl for smaller factors first
 */
ll ways[MX];
int main() {
        setIO();
        ...
        For(i,sz(v)){
                ways[i]=n/v[i];//?
                For(j,i){//see if any larger factors already included
                        ll y=v[j], x=v[i];
                        if(y%x==0){
                                ways[i]-=ways[j];
                        }
        ...
        For(i,sz(v))dbg(i,v[i],ways[i]);
        ll ans=0;
        For(i,sz(v)){
                ans+=ways[i]*modPow(2,v[i],MOD);
                ans%=MOD;
        }
        ans -= n-2;
        ans=(ans%MOD+MOD)%MOD;
