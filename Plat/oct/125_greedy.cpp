...
/*
first transform a[] by a[i]-n-a[i]
stops at first pos where (cnt<=i)>=i
bsearch on when this happens for each i
"walking on seg tree"
 */
int c[MX], mx;
vi upd[MX];
...
int main() {
        ...
        FOR(i,1,n+1){
                trav(x,upd[i])bit.upd(x+1,1);
                int l=1,r=n+1;
                while(l!=r){
                        int mid=(l+r)/2;
                        if(bit.sum(mid)>=i)r=mid;
                        else l=mid+1;
                }
                ckmax(mx,n-l);
        }
